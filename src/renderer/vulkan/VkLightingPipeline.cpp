...
void VkLightingPipeline::createDescriptorSetLayout() {
    ...
    bindings[21].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    bindings[23].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    bindings[27].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    for (uint32_t binding = 28; binding <= 30; ++binding)
        bindings[binding].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    bindings[31].descriptorCount =
        Iridium::kMaximumGpuReflectionProbeEnvironments;
...
void VkLightingPipeline::createPipeline(VkRenderPass renderPass,
    ...
    if (gBufferLayout == Iridium::GBufferLayout::CanonicalReference) {
        fragmentPath = "assets/shaders/canonical_reference_lighting_frag.spv";
    }
    else {
        fragmentPath = "assets/shaders/canonical_packed_lighting_frag.spv";
    }
    auto fragCode = readFile(std::string(PROJECT_ROOT_DIR) + fragmentPath);
