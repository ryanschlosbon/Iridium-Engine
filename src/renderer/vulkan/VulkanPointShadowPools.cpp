...
void VulkanPointShadowPools::init(VkDevice device,
    ...
    try {
        ...
        for (uint32_t tier = 0; tier < pools_.size(); ++tier) {
            ...
            pool.capacity = capacities[tier];
            const uint32_t layers = pool.capacity * 6u;
            pool.image = allocator.createImage2D(
                { pool.resolution, pool.resolution }, VK_FORMAT_D32_SFLOAT,
                VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT |
                    VK_IMAGE_USAGE_SAMPLED_BIT,
                VK_IMAGE_ASPECT_DEPTH_BIT,
                ProfileMemoryCategory::ShadowLocal, 1, layers,
            ...
            for (uint32_t layer = 0; layer < layers; ++layer) {
                ...
                view.image = pool.image.image;
                view.viewType = VK_IMAGE_VIEW_TYPE_2D;
                view.format = pool.image.format;
        ...
        // The shader performs the depth comparison. Interpolating projected
        // depth first stretches discontinuities; explicit PCF/PCSS taps own
        // the intended smoothing instead.
        ...
        requireSuccess(vkCreatePipelineLayout(device_, &pipelineLayout,
        ...
        for (uint32_t frame = 0; frame < frameBuffers_.size(); ++frame) {
            ...
            const VkDescriptorBufferInfo buffer{ frameBuffers_[frame].buffer,
                0, sizeof(VulkanPointShadowData) };
        ...
        for (Pool& pool : pools_)
            uploads.enqueueTransition(pool.image, ResourceState::ShaderResource);
    } catch (...) {
        cleanup();
        throw;
    }
...
void VulkanPointShadowPools::cleanup() noexcept {
...
void VulkanPointShadowPools::updateFrame(uint32_t frameIndex,
    std::span<const PointShadowFramePacket> packets) {
    if (frameIndex >= frameBuffers_.size() ||
        packets.size() > kPointShadowEntryCapacity)
        throw std::out_of_range("Point shadow frame data is invalid");
    VulkanPointShadowData data{};
    std::array<bool, kPointShadowEntryCapacity> occupied{};
    uint32_t activeCount = 0;
    for (const PointShadowFramePacket& packet : packets) {
        const uint32_t tier = poolIndex(packet.resolution);
        const bool finitePosition = std::isfinite(packet.lightPosition.x) &&
            std::isfinite(packet.lightPosition.y) &&
            std::isfinite(packet.lightPosition.z);
        if (packet.shadowDataSlot >= kPointShadowEntryCapacity ||
            occupied[packet.shadowDataSlot] ||
            packet.cubeIndex >= capacities_[tier] ||
            !finitePosition ||
            !(packet.farPlane > packet.nearPlane) || packet.nearPlane <= 0.0f)
            throw std::invalid_argument("Point shadow packet is invalid");
        occupied[packet.shadowDataSlot] = true;
        VulkanPointShadowEntry& entry = data.entries[packet.shadowDataSlot];
        entry.worldToShadowClip = packet.worldToShadowClip;
        entry.lightPositionFar = glm::vec4(packet.lightPosition,
            packet.farPlane);
        entry.metadata = { packet.lightSlot, packet.sampleable ? 1u : 0u,
            tier, packet.cubeIndex };
        const float denominator = packet.farPlane - packet.nearPlane;
        entry.depthBias = {
    ...
    data.metadata = { activeCount, capacities_[0], capacities_[1],
        capacities_[2] };
...
VkDescriptorSet VulkanPointShadowPools::renderDescriptor(
    ...
    if (frameIndex >= renderSets_.size())
        throw std::out_of_range("Point shadow descriptor frame is invalid");
    return renderSets_[frameIndex];
}

std::array<VkDescriptorImageInfo, 3>
VulkanPointShadowPools::sampleImages() const noexcept {
    std::array<VkDescriptorImageInfo, 3> result{};
    for (uint32_t tier = 0; tier < result.size(); ++tier)
        result[tier] = { sampler_, pools_[tier].image.view,
            VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL };
    return result;
}

VkDescriptorBufferInfo VulkanPointShadowPools::sampleBuffer(
...
VkShaderModule VulkanPointShadowPools::createShaderModule(
    ...
    requireSuccess(vkCreateShaderModule(device_, &create, nullptr, &module),
        "vkCreateShaderModule(point shadow)");
    return module;
}
...
VkPipeline VulkanPointShadowPools::createPipeline(bool alphaMasked,
    ...
    try {
        ...
        raster.cullMode = doubleSided ? VK_CULL_MODE_NONE :
        ...
        dynamic.dynamicStateCount =
            static_cast<uint32_t>(dynamicValues.size());
